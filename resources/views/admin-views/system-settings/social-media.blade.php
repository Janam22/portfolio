                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control w-100" name="name" id="name">
                                            ...
                                            <option value="github">{{translate('messages.Github')}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
...
@push('script_2')
    ...
    <script>
        ...
        function fetch_social_media() {
            ...
            $.ajax({
                url: "{{route('admin.system-settings.social-media.fetch')}}",
                method: 'GET',
        ...
        $('#add').on('click', function () {
            ...
            var link = $('#link').val();
            if (name == "") {
                toastr.error('{{translate('messages.Social Name Is Required')}}.');
                return false;
            }
            if (link == "") {
                toastr.error('{{translate('messages.Social Link Is Required')}}.');
                return false;
            }
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                url: "{{route('admin.system-settings.social-media.store')}}",
                method: 'POST',
                data: {
                    name: name,
                    link: link
        ...
        $('#update').on('click', function () {
            ...
            $.ajax({
                url: "{{url('admin/system-settings/social-media')}}/"+id,
                method: 'PUT',
                data: {
                    id: id,
                    name: name,
        ...
        $(document).on('click', '.delete', function () {
            ...
            if (confirm("{{translate('messages.Are you sure delete this social media')}}?")) {
                ...
                $.ajax({
                    url: "{{url('admin/system-settings/social-media/destroy')}}/"+id,
                    method: 'POST',
                    data: {id: id},
                    success: function (data) {
                        fetch_social_media();
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.system-settings.social-media.status-update')}}",
                method: 'get',
                data: {
                    id: id,
                    status: status
